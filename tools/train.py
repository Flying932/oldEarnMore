...
def main():
    ...
    if vector_env_type not in ("sync", "async"):
        print(f"[warn] unknown vector_env_type={vector_env_type}, fallback to async")
        vector_env_type = "async"

    print(50 * "-" + f"build train enviroment ({vector_env_type})" + "-" * 50)
    ...
    print(50 * "-" + f"build val enviroment ({vector_env_type})" + "-" * 50)
    cfg.environment.update(dict(
        mode="val",
        if_norm = True,
        dataset = dataset,
    ...
    print(f"[runtime] buffer_device={rb_device}, num_envs={cfg.num_envs}, batch_size={cfg.batch_size}, repeat_times={cfg.repeat_times}, horizon_len={cfg.horizon_len}")
